...
export async function getCollectionItems({
  ...
  try {
    ...
    // CORRECTION ICI : On sélectionne 'type' au lieu de la table 'product_categories'
    let query = supabase
      .from('user_collection')
      .select(`
        id,
    ...
    // @ts-ignore - Le typage Supabase auto-généré peut être strict
    const items: CollectionItem[] = data.map(item => ({
      ...
      product: {
        ...
        // HACK: On mappe le 'type' (ENUM) vers l'objet 'category' attendu par l'UI
        // @ts-ignore
        category: { name: item.products.type || 'OTHER' }
      },
      quantity: item.quantity,
      condition: item.condition as any, // Cast condition to specific enum if needed
